      ...
      <div class="col-lg-9 main-chart">
            ...
            <div class="box1">
              ...
              <h3>
                ...
                $sql = "SELECT table_schema \"db_puskesmas\",
                            ...
                            FROM information_schema.tables WHERE table_schema = 'db_puskesmas' GROUP BY table_schema";
        ...
        <div class="row mt">
          ...
          <div class="custom-bar-chart">
            ...
            $konek = mysqli_connect("localhost:3307", "root", "root", "db_puskesmas");
            ...
            for ($i = 0; $i <= 6; $i++) {
              ...
              $result_pengujung = mysqli_query($konek, "SELECT * FROM statistik WHERE tanggal='$hasil_pengujung'");
              if (!$result_pengujung) {
                die("Error in query: " . mysqli_error($konek));
              }
              $query_pengujung = mysqli_num_rows($result_pengujung);

              $tgl_hits   = strtotime("+$i day", strtotime($hasilnya));
              $hasil_hits = date("Y-m-d", $tgl_hits);

              // Check if the query executes successfully
              $result_hits = mysqli_query($konek, "SELECT SUM(hits) as hitstoday FROM statistik WHERE tanggal='$hasil_hits' GROUP BY tanggal");
              if (!$result_hits) {
                die("Error in query: " . mysqli_error($konek));
              }
              $query_hits = mysqli_fetch_array($result_hits);
...
<footer class="site-footer">
  <div class="text-center">
    <?php echo date("Y"); ?> - Theme by Pendaman Bohalima
